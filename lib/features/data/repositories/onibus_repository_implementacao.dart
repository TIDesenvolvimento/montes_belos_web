...
class OnibusRepositoryImplementacao extends OnibusRepository {
  OnibusDataSource onibusDataSource;

  OnibusRepositoryImplementacao(this.onibusDataSource);

  @override
  Future<Either<Exception, Onibus>> getLayoutDoOnibus(
      OnibusRequest onibusRequest) async {
    try {
      final result = await onibusDataSource.getLayoutDoOnibus(onibusRequest);
      if (result.busLayoutDto != null) {
        return Right(getOnibus(result));
      }
