...
class VoltaPage extends StatefulWidget {
  ...
  VoltaPage({Key? key}) : super(key: key);

  @override
  _VoltaPageState createState() => _VoltaPageState();

  VoltaPage.fromArgs(dynamic arguments) {
...
class _VoltaPageState extends State<VoltaPage> {
    ...
    return Container(
        ...
        child: Column(
          children: [
            ...
            widget.dataVolta != '' ? buildCalendario() : Container(),
            Container(
              height: 1,
                            ...
                            child: SizedBox(
                              ...
                              child: ElevatedButton(
                                ...
                                onPressed: quantidadeDePoltronasSelecionadas > 0
                                    ? () async {
                                        if (purchaseRequest.forwards == null) {
                                          mostrarSnackBarAtencao(
                                              'Selecione a ida antes de selecionar a volta');
                                          return;
                                        }
  ...
  buildCalendario() {
    return DatePickerViagem(
      DateTime.parse(widget.dataVolta!)
        ...
        child: Column(
          children: [
            Row(
              ...
              children: [
                MouseRegion(
                  cursor: purchaseRequest.forwards == null
                      ? SystemMouseCursors.click
                      : SystemMouseCursors.basic,
                  child: GestureDetector(
                    onTap: purchaseRequest.forwards != null
                        ? null
                        : () {
                            Navigator.of(context).pop();
                          },
                    child: _buildStep('SELECIONAR IDA', 1, currentStep),
                  ),
                ),
                _buildDivider(),
                _buildStep('SELECIONAR VOLTA', 2, currentStep),
                _buildDivider(),
