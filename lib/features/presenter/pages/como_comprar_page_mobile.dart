...
class ComoComprarPageMobile extends StatefulWidget {
  @override
  _ComoComprarPageMobileState createState() => _ComoComprarPageMobileState();
}

class _ComoComprarPageMobileState extends State<ComoComprarPageMobile> {
  double largura = 0.0;
  ...
  final String fixedRecipient = 'jorgematheus.p@gmail.com';
  ...
  Future<void> sendEmail() async {
    String username = 'front@tisistema.com.br'; // Seu e-mail da Hostinger
    String password = 'T+i=@Front@_0_3***'; // Senha do e-mail

    final smtpServer = SmtpServer('smtp.hostinger.com',
        port: 465, // Use 465 para SSL
        username: username,
        password: password,
        ssl: false); // Use true para SSL

    final emailMessage = Message()
      ..from =
          Address(username, nomeTextEditingController.text) // Nome do usuário
      ..recipients.add(fixedRecipient) // Destinatário fixo
      ..subject = assuntoTextEditingController.text // Assunto do e-mail
      ..text =
          'Nome: ${nomeTextEditingController.text}\nE-mail: ${emailTextEditingController.text}\nMensagem: ${mensagemTextEditingController.text}';

    try {
      final sendReport = await send(emailMessage, smtpServer);
      print('E-mail enviado: ' + sendReport.toString());
      // Exiba uma mensagem de sucesso ou faça outra ação, se necessário
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('E-mail enviado com sucesso!')));
    } on MailerException catch (e) {
      print('E-mail não enviado. ${e.toString()}');
      // Exiba uma mensagem de erro ou faça outra ação, se necessário
  ...
  void _submitForm() {
    if (nomeTextEditingController.text.isNotEmpty &&
      ...
      mostrarSnackBarFalha('Por favor, preencha todos os campos.');
    }
  }
}
